...
export class SSHSessionDO implements DurableObject {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (!sftpAttachToken || !SFTP_ATTACH_TOKEN_PATTERN.test(sftpAttachToken)
      || !sftpAttachUrl || !this.isValidAttachUrl(sftpAttachUrl, sftpAttachToken, '/api/admin/ssh/sftp')
      || !processAttachToken || !SFTP_ATTACH_TOKEN_PATTERN.test(processAttachToken)
      || !processAttachUrl || !this.isValidAttachUrl(processAttachUrl, processAttachToken, '/api/admin/ssh/processes')) {
  ...
  private isValidAttachUrl(value: string, token: string, pathname: '/api/admin/ssh/sftp' | '/api/admin/ssh/processes'): boolean {
