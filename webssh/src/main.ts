...
async function issueTicket(signal: AbortSignal): Promise<{ ticket: string; sessionId: string }> {
  // 会话票据接口受管理员会话保护：/api/admin/* 的非安全方法要求携带
  // X-CSRF-Token（与 cf_monitor_csrf Cookie 一致），否则返回 403。
  ...
  const response = await fetch('/api/admin/ssh/session', {
    method: 'POST',
    headers,
